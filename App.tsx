...
const AIPlannerIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>;
const AgentIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>;
const SimIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>; // Reuse for now or find better
...
export const App = () => {
  ...
  const handleClearGanttFilters = () => {
      setGanttFilters({ program: [], subProgram: [], operator: [], showCriticalOnly: false });
  };
  ...
  const handleDashboardFilterChange = (filterType: keyof typeof dashboardFilters, value: string | boolean) => {
      setDashboardFilters(prev => {
          ...
          const list = prev[filterType as 'status' | 'operator'] as any[];
          return {
              ...prev,
              [filterType]: list.includes(value) ? list.filter(v => v !== value) : [...list, value]
          };
      });
  };
  ...
  const navItems = [
    ...
    { id: 'aiPlanner', label: t('nav.aiPlanner'), icon: <AIPlannerIcon /> },
    { id: 'aiAgent', label: t('nav.aiAgent'), icon: <AgentIcon /> },
    { id: 'simulator', label: t('nav.processSimulator') || 'Process Simulator', icon: <SimIcon /> },
  ...
  const renderMainContent = () => {
    ...
    switch (currentView) {
      ...
      case 'aiPlanner': return ( <AIPlannerPage onUpdateTasks={handleSetTasks} allTasks={tasks} allOperators={operators} masterTasks={masterTasks} rankSettings={rankSettings} /> );
      case 'aiAgent': return ( <AIAgentPage onUpdateTasks={handleSetTasks} onUpdateMasterTasks={handleSetMasterTasks} allTasks={tasks} masterTasks={masterTasks} /> );
      case 'simulator': return ( <SimulatorPage onUpdateTasks={handleSetTasks} allTasks={tasks} allOperators={operators} masterTasks={masterTasks} /> );
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 overflow-hidden font-sans transition-colors duration-300">
      ...
      <nav 
        ...
        className={`bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800/50 flex-shrink-0 transition-all duration-300 flex flex-col ${isGanttInteracting ? 'w-0 opacity-0 pointer-events-none' : isSidebarCollapsed ? 'w-20' : 'w-64'}`}
      >
        <div className={`px-6 py-8 flex items-center bg-slate-100 dark:bg-[#0e0e2f] ${isSidebarCollapsed ? 'justify-center' : 'gap-4'}`}>
          <div className="w-10 h-10 bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center flex-shrink-0 rounded-xl shadow-lg shadow-cyan-500/20 group-hover:scale-110 transition-transform duration-300">
          ...
          {!isSidebarCollapsed && (
            <div className="flex flex-col">
                <span className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 uppercase tracking-[0.4em] leading-none mb-1">Apex Aero</span>
                <span className="text-xl font-bold text-slate-900 dark:text-white tracking-tight uppercase leading-none flex items-center">
        ...
        <div className="flex-1 overflow-y-auto py-2 px-4 space-y-2 custom-scrollbar">
          {navItems.map(item => (
            <button
              key={item.id}
            ...
            >
              ...
              <span className={`transition-colors flex-shrink-0 ${currentView === item.id ? 'text-cyan-400' : 'text-slate-600 group-hover:text-slate-400'}`}>
                {item.icon}
              </span>
              {!isSidebarCollapsed && <span className="text-xs font-bold uppercase tracking-widest truncate">{item.label}</span>}
            </button>
          ))}
        </div>
        
        <div className="p-6 border-t border-slate-200 dark:border-slate-800/50 flex flex-col gap-4">
            {!isSidebarCollapsed && (
                <div className="font-mono text-[9px] text-slate-400 dark:text-slate-600 uppercase tracking-widest">
                    Build 2.1.0-stable
                </div>
            )}
            <button 
                onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
                className="w-full flex items-center justify-center p-2 text-slate-400 dark:text-slate-600 hover:text-slate-900 dark:hover:text-white transition-colors"
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-900 relative transition-colors duration-300">
        <DashboardHeader 
