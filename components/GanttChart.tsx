...
const GanttChart: React.FC<GanttChartProps> = ({ tasks, allTasks, allOperators, onTaskSelect, onOperatorSelect, selectedTaskId, onTaskUpdate, columnVisibility, onGanttInteractionChange, onTaskReorder, masterTasks, filters, onFilterChange, onClearFilters, hourWidth, onZoomIn, onZoomOut, onFitWeek }) => {
  ...
  const renderCellContent = (task: Task, columnId: string) => {
    ...
    switch (columnId) {
        ...
        case 'realisation':
            ...
            return (
              <span className={`font-medium ${textColor}`}>
                {t('gantt.realisation.hours', realisedHours.toFixed(1))}
              </span>
            );
        case 'assignedTo':
            return (
        ...
        case 'duration':
            return <span className="text-slate-600 dark:text-slate-300 font-medium text-[10px]">{t('gantt.duration.hours', task.duration)}</span>;
        default:
            return null;
    }
  };
            ...
            >
                ...
                <div className="flex flex-shrink-0 bg-white dark:bg-slate-900">
                    {visibleColumns.map((col) => (
                        <div key={col.id} title={col.label} className={`group relative font-bold text-slate-500 dark:text-slate-500 text-[9px] uppercase tracking-widest p-3 flex flex-col items-center justify-center text-center border-b border-r border-slate-200 dark:border-slate-800/50 header-cell bg-slate-50 dark:bg-slate-900`} style={{width: columnWidths[col.id], height: HEADER_HEIGHT}}>
                            <div className="flex items-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                {col.icon}
                            </div>
                            <span className="transition-colors group-hover:text-cyan-600 dark:group-hover:text-cyan-400 mt-1">{col.label}</span>
                            <div
