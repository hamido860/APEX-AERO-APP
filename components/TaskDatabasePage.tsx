                            ...
                            {isEditing && editingTask ? (
                                <>
                                    ...
                                    {columnVisibility.actions && <td className="p-2">
                                        <div className="flex items-center gap-2">
                                            <button onClick={handleSaveEdit} aria-label={t('taskdatabase.actions.save')} title={t('taskdatabase.actions.save')} className="p-2 rounded text-slate-200 bg-green-600 hover:bg-green-500 transition-colors"><SaveIcon /></button>
                                            <button onClick={handleCancelEdit} aria-label={t('taskdatabase.actions.cancel')} title={t('taskdatabase.actions.cancel')} className="p-2 rounded text-slate-200 bg-slate-600 hover:bg-slate-500 transition-colors"><CancelIcon /></button>
                                        </div>
                                    </td>}
                                </>
                            ) : (
                                <>
                                    ...
                                    {columnVisibility.actions && <td className="p-2">
                                        <div className="flex items-center gap-2">
                                            <button onClick={() => handleEdit(task)} aria-label={t('taskdatabase.actions.edit')} title={t('taskdatabase.actions.edit')} className="p-2 rounded text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-colors"><EditIcon /></button>
                                            <button onClick={() => handleDelete(task.name)} aria-label={t('taskdatabase.actions.delete')} title={t('taskdatabase.actions.delete')} className="p-2 rounded text-slate-500 dark:text-slate-400 hover:bg-red-600/80 hover:text-white transition-colors"><DeleteIcon /></button>
                                        </div>
                                    </td>}
                                </>
                            )}
